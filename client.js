...
client.on('data', (data) => {
  ...
  //기존 코드에서 마이그레이션
  //const packet = data.slice(totalHeaderLength, length);
  //slice가 deprecated 돼서 subarray로 사용하게 되었는데 slice는 시작인덱스부터 몇개의 데이터를 잘라올 건지라면,
  //subarray는 시작인덱스 ~ 끝인덱스로 표기하기에 그에 맞춰서 수정했습니다
  //기존 코드에선 out of index 나옵니다.
  const packet = data.subarray(
    totalHeaderLength,
    totalHeaderLength + (length - PACKET_TYPE_LENGTH),
  ); // 패킷 데이터
